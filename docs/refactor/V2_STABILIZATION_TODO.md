...
1. `/orbit-status` must parse paper package status and human decision verdicts instead
...
2. Codex standalone handoff must preserve and restore producer workflow context.
...
3. Diagnostic sessions need clearer rerun, resume, and fresh-run semantics.
...
4. Residual pre-v2 paper-writing / CLAIM_CONSTRUCTION wording must be cleaned or
   archived.
...
5. Test suite needs a fast stabilization test target.
...
6. Version terminology needs one documented convention.
