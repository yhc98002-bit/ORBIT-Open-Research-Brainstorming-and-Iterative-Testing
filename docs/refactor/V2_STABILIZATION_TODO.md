...
1. STOP C approval checker must reject draft, pending, or non-gating claim ledgers.
   - Current risk: a ledger that is structurally valid but not commitment-ready could be
     treated as satisfying downstream paper gates.
   - Expected stabilization: `/paper-from-claims`, `/submission-package`, and validator
     helpers must distinguish approved claim ledgers from draft or Codex-pending ledgers.

2. Verdict parser must support Markdown single-token verdicts and reject candidate
   lists/templates.
   - Current risk: parser logic can mistake a template list of possible verdicts for an
     actual final verdict.
   - Expected stabilization: accept a final standalone token, but reject examples,
     tables, candidate lists, and templates.

3. `/orbit-status` must parse paper package status and human decision verdicts instead
   of relying on file presence.
   - Current risk: blocked or draft `paper/paper_package.json` may be interpreted as
     completed because the file exists.
   - Expected stabilization: status inference should read package `status`, STOP C
     approval verdicts, and blocker fields.
...
7. Test suite needs a fast stabilization test target.
...
8. Version terminology needs one documented convention.
