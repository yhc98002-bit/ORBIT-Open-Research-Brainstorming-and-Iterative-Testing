...
- **FETCH_SCRIPT** — `tools/exa_search.py` in the current project, or the same path under the ARIS/ORBIT repo recorded in `.aris/installed-skills.txt`.
...
SCRIPT="tools/exa_search.py"
[ -f "$SCRIPT" ] || { [ -n "${ARIS_REPO:-}" ] && SCRIPT="$ARIS_REPO/tools/exa_search.py"; }
[ -f "$SCRIPT" ] || SCRIPT=""
```
...
if [ -d research-wiki/ ] and query category was "research paper":
    ...
    [ -f "$WIKI_SCRIPT" ] || {
      echo "WARN: research_wiki.py not found; exa-search results delivered, wiki ingest skipped. Fix: bash tools/install_aris.sh, export ORBIT_REPO/ARIS_REPO, or cp <ARIS-repo>/tools/research_wiki.py tools/." >&2
      WIKI_SCRIPT=""
    }
