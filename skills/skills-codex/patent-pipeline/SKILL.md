...
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, WebSearch, WebFetch, Agent, Skill, spawn_agent
---
...
- **REVIEWER_MODEL = `gpt-5.5`** — Model used via Codex-native sub-agent for examiner-style review.
...
- If `spawn_agent` is not available (no OpenAI API key), skip external cross-model review and note it in the output. The pipeline must not fail due to missing reviewer access.
