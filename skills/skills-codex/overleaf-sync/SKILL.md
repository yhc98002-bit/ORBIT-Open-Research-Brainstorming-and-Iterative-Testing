...
First resolve the setup helper in the project shell:
...
OVERLEAF_SETUP=".aris/tools/overleaf_setup.sh"
[ -f "$OVERLEAF_SETUP" ] || OVERLEAF_SETUP="tools/overleaf_setup.sh"
if [ ! -f "$OVERLEAF_SETUP" ]; then
  if [ -n "${ORBIT_REPO:-}" ] && [ -f "$ORBIT_REPO/tools/overleaf_setup.sh" ]; then
    OVERLEAF_SETUP="$ORBIT_REPO/tools/overleaf_setup.sh"
  elif [ -n "${ARIS_REPO:-}" ] && [ -f "$ARIS_REPO/tools/overleaf_setup.sh" ]; then
    OVERLEAF_SETUP="$ARIS_REPO/tools/overleaf_setup.sh"
  fi
fi
[ -f "$OVERLEAF_SETUP" ] || {
  echo "ERROR: overleaf_setup.sh not found at .aris/tools/, tools/, \$ORBIT_REPO/tools/, or \$ARIS_REPO/tools/." >&2
  exit 1
}
```
...
Run this in your own terminal (NOT through me):
    ...
    bash "$OVERLEAF_SETUP" <project-id-or-url>
...
# Resolve audit helper with the same helper-resolution chain:
if [ -z "${ARIS_REPO:-}" ] && [ -f ../.aris/installed-skills.txt ]; then
  ARIS_REPO=$(awk -F'\t' '$1=="repo_root"{print $2; exit}' ../.aris/installed-skills.txt 2>/dev/null) || true
fi
OVERLEAF_AUDIT="../.aris/tools/overleaf_audit.sh"
[ -f "$OVERLEAF_AUDIT" ] || OVERLEAF_AUDIT="../tools/overleaf_audit.sh"
if [ ! -f "$OVERLEAF_AUDIT" ]; then
  if [ -n "${ORBIT_REPO:-}" ] && [ -f "$ORBIT_REPO/tools/overleaf_audit.sh" ]; then
    OVERLEAF_AUDIT="$ORBIT_REPO/tools/overleaf_audit.sh"
  elif [ -n "${ARIS_REPO:-}" ] && [ -f "$ARIS_REPO/tools/overleaf_audit.sh" ]; then
    OVERLEAF_AUDIT="$ARIS_REPO/tools/overleaf_audit.sh"
  fi
fi
[ -f "$OVERLEAF_AUDIT" ] || { echo "ERROR: overleaf_audit.sh not found" >&2; exit 1; }
bash "$OVERLEAF_AUDIT" .   # must report "Audit clean"
```
