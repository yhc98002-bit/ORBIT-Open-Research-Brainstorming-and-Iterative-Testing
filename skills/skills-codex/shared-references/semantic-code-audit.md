...
1. Verdict on its own line, exactly one of:
   ...
   "incomplete_input_set". Codex availability failures are not represented as an audit
   verdict under the default policy; follow `codex-precondition.md` and stop with STATE
...
- `ERROR` (audit could not complete because required inputs are incomplete or a regime
  ...
  Codex unavailability is handled before audit generation by `codex-precondition.md`
