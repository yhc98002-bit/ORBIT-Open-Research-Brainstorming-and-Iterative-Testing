...
- `$ORBIT_REPO/tools/<helper>` or `$ARIS_REPO/tools/<helper>` — explicit repo env vars; `ARIS_REPO` may also be auto-resolved from the install manifest

So callers MUST use the standard resolver in
[`helper-resolution.md`](helper-resolution.md), not a hard-coded path. Short form:
...
if [ ! -f "$HELPER" ]; then
  if [ -n "${ORBIT_REPO:-}" ] && [ -f "$ORBIT_REPO/tools/<helper>" ]; then
    HELPER="$ORBIT_REPO/tools/<helper>"
  elif [ -n "${ARIS_REPO:-}" ] && [ -f "$ARIS_REPO/tools/<helper>" ]; then
    HELPER="$ARIS_REPO/tools/<helper>"
  fi
fi
[ -f "$HELPER" ] || { echo "WARN: <helper> not found; rerun bash tools/install_aris.sh, set ORBIT_REPO/ARIS_REPO, or copy the helper to tools/. Skipping this step." >&2; HELPER=""; }
...
- ✅ `bash "$VERIFY_AUDITS" <paper> --assurance submission` after resolving `VERIFY_AUDITS` via [`helper-resolution.md`](helper-resolution.md)
