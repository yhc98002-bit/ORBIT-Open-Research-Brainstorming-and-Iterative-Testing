...
```jsonc
{
  ...
  "status": "awaiting_human_continue",         // one of: in_progress | awaiting_human_continue | awaiting_user_action | completed
...
| `awaiting_user_action` | Skill cannot continue until the user fixes an external prerequisite or explicitly chooses a degraded path | Written for blocked dependencies such as Codex auth/MCP setup failure, missing credentials, unavailable reviewer backend, or a required manual file that the agent must not fabricate | Same skill re-invoked: first verify the prerequisite is fixed or an explicit override is present, then transition to `in_progress` and resume from `next_action`. Downstream skills must not treat this as approval. |
...
The third state (`awaiting_human_continue`) is what the v1.0/v1.1 schema lacked. The fourth
state (`awaiting_user_action`) keeps dependency failures distinct from both intentional
human checkpoints and resumable execution. Without it, a Codex/auth/setup failure can be
mistaken for approval to continue or for a stale interrupted run.
...
2. **`in_progress` is the default during execution.** Do not write `completed` until the
   ...
   be invoked). Do not write `awaiting_user_action` unless an external prerequisite or
   explicit user-authored artifact is required before the skill can safely continue.
...
5. **Status transitions are unidirectional within a run.**
   ...
   `in_progress` → `awaiting_user_action` → `in_progress` is allowed after the user fixes
   the blocked prerequisite or passes an explicit override.
...
5. status = "awaiting_user_action":
   - Same skill being re-invoked → verify the prerequisite is fixed or an explicit override
     is present. If fixed, transition to in_progress and resume from next_action. If not,
     print the remediation and exit without writing downstream artifacts.
   - Downstream skill being invoked → block. Do not interpret this status as approval to
     continue; route the user back to the blocked skill or prerequisite.
```
...
3. **Routing by status:**
   ...
   - If any upstream STATE has `status = "awaiting_user_action"` → block downstream routing
     unless the user supplied an explicit override accepted by the downstream skill.
     Surface the upstream `next_action` and remediation.
...
- [ ] If the skill hits a blocked external prerequisite, set
      `status = "awaiting_user_action"` with a precise `next_action`; do not emit a
      substitute artifact that downstream gates might mistake for valid output.
...
- `status` enum gains `awaiting_human_continue` and `awaiting_user_action` (previously
...
- **Status = "running" or "active".** Use `in_progress` exactly. The fixed enum is
