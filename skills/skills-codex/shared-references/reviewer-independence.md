...
- Any skill that sends artifacts to `spawn_agent` or `send_input`

## Exception

Multi-round review within the SAME thread (`send_input`) may reference the reviewer's own previous feedback to check resolution — but still must not include executor interpretations of that feedback.
