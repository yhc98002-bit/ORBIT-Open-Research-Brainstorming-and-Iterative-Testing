...
- `orbit-research/RED_TEAM_REVIEW.md`  *(must end `READY_FOR_PAPER`)*
- `orbit-research/HUMAN_DECISION_NOTE.md`  *(required by G19 — must be human-authored or human-confirmed and end `PROCEED`)*
...
If `RED_TEAM_REVIEW.md` is missing, stop and route the user back to `/auto-review-loop`
through `/diagnostic-to-review` STOP C. If it exists but does not end `READY_FOR_PAPER`,
stop and route to the blocker named by the review verdict; do not begin Phase 0.

If `HUMAN_DECISION_NOTE.md` is missing, stop and ask for explicit human STOP C approval.
`/result-to-claim` may write `AGENT_DECISION_RECOMMENDATION.md`, but that recommendation
does not satisfy G19. If `HUMAN_DECISION_NOTE.md` exists but its final verdict is not
`PROCEED`, stop and follow that verdict (`NARROW`, `REDESIGN`, `RE-READ`,
`CHANGE BENCHMARK`, `STOP`, or `HUMAN_DECISION_REQUIRED`). Do not begin Phase 0 until the
human decision authorizes paper writing.
...
Resolve the external verifier before printing/running the checklist:
...
VERIFY_AUDITS=".aris/tools/verify_paper_audits.sh"
[ -f "$VERIFY_AUDITS" ] || VERIFY_AUDITS="tools/verify_paper_audits.sh"
if [ ! -f "$VERIFY_AUDITS" ]; then
  if [ -n "${ORBIT_REPO:-}" ] && [ -f "$ORBIT_REPO/tools/verify_paper_audits.sh" ]; then
    VERIFY_AUDITS="$ORBIT_REPO/tools/verify_paper_audits.sh"
  elif [ -n "${ARIS_REPO:-}" ] && [ -f "$ARIS_REPO/tools/verify_paper_audits.sh" ]; then
    VERIFY_AUDITS="$ARIS_REPO/tools/verify_paper_audits.sh"
  fi
fi
[ -f "$VERIFY_AUDITS" ] || {
  echo "ERROR: verify_paper_audits.sh not found at .aris/tools/, tools/, \$ORBIT_REPO/tools/, or \$ARIS_REPO/tools/." >&2
  exit 1
}
```
...
`.aris/tools/verify_paper_audits.sh` → project `tools/verify_paper_audits.sh`
→ `$ORBIT_REPO/tools/verify_paper_audits.sh` → `$ARIS_REPO/tools/verify_paper_audits.sh`.
...
```bash
bash "$VERIFY_AUDITS" paper/ --assurance submission
```
...
This hook example assumes `ORBIT_REPO` is exported in the hook environment.
It is documented here, not required. Phase 6.0's verifier-as-truth pattern is
the default repo behavior.
