...
- **REVIEWER_MODEL = `gpt-5.5`** — Model used via a secondary Codex agent for plan review, figure review, writing review, and improvement loop.
...
**Round 1:** GPT-5.5 xhigh reviews the full paper → identifies CRITICAL/MAJOR/MINOR issues → Codex implements fixes → recompile → save `main_round1.pdf`

**Round 2:** GPT-5.5 xhigh re-reviews with conversation context → identifies remaining issues → Codex implements fixes → recompile → save `main_round2.pdf`
