...
- `../shared-references/innovation-loops.md` — Claude CLI collaborative-mode prompt template (§7.1) for Stage 8/9/10/18.5 re-runs
- `../shared-references/continuation-contract.md` — STATE.json schema, four-state enum, cross-skill resume rules
...
**Claude CLI reviewer unavailability — LOUD STOP, not silent fallback:**
Follow [`../shared-references/claude-cli-review.md`](../shared-references/claude-cli-review.md).
...
- If a Claude CLI review call fails mid-Phase-1 (§5 of the precondition contract):
  capture the error, update STATE with `status: "awaiting_user_action"`
  and `claude_cli_call_failure`, surface the verbatim user-facing remediation
  message, and stop. Already-revised critique items keep their updates;
  the failing critique item gets `addressed = "blocked: claude_cli_call_failure"`
...
- Override: `— claude-required: false` opts into the degraded single-model
...
**Claude CLI reviewer unavailability in Phase 3 — LOUD STOP, not silent skip:**
...
- The Phase 0 precondition (§3 of
  [`../shared-references/claude-cli-review.md`](../shared-references/claude-cli-review.md))
...
- If a Codex re-evaluation call fails mid-Phase-3: capture the error, set
  STATE `status: "awaiting_user_action"` with `claude_cli_call_failure`, write
...
- Override: `— claude-required: false` opts into a degraded run where Phase 3
...
Claude CLI reviewer unavailability follows the **Codex Precondition + Loud-Stop
Contract** in [`../shared-references/claude-cli-review.md`](../shared-references/claude-cli-review.md):

- **Phase 0 precondition.** Check Claude CLI availability at skill entry (§3 of
  the contract); LOUD STOP at `phase-0-precondition` if unavailable.
- **Mid-run failure.** Any failing `claude -p` call during Phase 1
...
- **Override.** `— claude-required: false` opts into degraded single-model
...
/ Load-bearing degradation) is **deprecated** for Claude CLI availability — every
tier collapses to "LOUD STOP unless `— claude-required: false`". The cited
