...
Use the external LLM via Claude CLI reviewer for divergent thinking:
...
Save the Claude session ID for follow-up.
...
2. **Critical review**: Use Claude CLI via `claude -p --resume` (same resumed Claude CLI session):
   ```
