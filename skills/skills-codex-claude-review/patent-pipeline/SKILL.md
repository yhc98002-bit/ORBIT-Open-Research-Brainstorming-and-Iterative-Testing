...
This runs 2 rounds of examiner-style review via Claude CLI max-effort. The examiner evaluates clarity, written description, enablement, novelty, non-obviousness, and claim scope.
...
- If `claude -p` is not available (no OpenAI API key), skip external cross-model review and note it in the output. The pipeline must not fail due to missing reviewer access.
