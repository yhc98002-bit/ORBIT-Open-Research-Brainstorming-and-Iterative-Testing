...
All ORBIT review calls use **Claude CLI reviewer** (`claude -p`) with `model: claude-cli` and `Claude CLI `--effort max``.
...
```toml
model = "claude-cli"
Claude CLI `--effort max` = "xhigh"
sandbox_mode = "danger-full-access"
```

## Optional: GPT-5.5 Pro via Oracle

When the user explicitly passes `— reviewer: oracle-pro`, route the review through Oracle MCP instead of Claude CLI reviewer.
...
If not specified OR `— reviewer: codex`:
    → Use spawn_agent with model: claude-cli and Claude CLI `--effort max`
