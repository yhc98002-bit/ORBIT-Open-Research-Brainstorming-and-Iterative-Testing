...
1. Verdict on its own line, exactly one of:
   ...
   (use ERROR only if the audit could not be completed because required inputs are
   missing or internally inconsistent. Include a reason code such as
   "incomplete_input_set". Claude CLI availability failures are not represented as an audit
   verdict under the default policy; follow `claude-cli-review.md` and stop with STATE
   `awaiting_user_action` without writing this audit verdict.)
...
- `ERROR` (audit could not complete because required inputs are incomplete or a regime
  ...
  Claude CLI reviewer unavailability is handled before audit generation by `claude-cli-review.md`
  (LOUD STOP, STATE `awaiting_user_action`, no substitute audit verdict).
