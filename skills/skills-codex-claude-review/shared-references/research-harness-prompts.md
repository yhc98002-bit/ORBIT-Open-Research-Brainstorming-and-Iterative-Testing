...
> Claude CLI collaborative-mode delegation. See the v1.0 → v1.3 alias appendix at the end.
>
> Reviewer routing default: Claude CLI reviewer, `model = claude-cli`, `reasoning_effort = xhigh`,
...
top-3 marked for Stage 9 expansion, and a "Claude CLI collaborative additions" section.
...
Bold analogies welcome. Claude CLI collaborative additions append more analogies.

Output: ANALOGY_TRANSFER.md with the mapping table, hybrid mechanisms suggested by the
analogies, and a Claude CLI collaborative additions section.
...
```text
You are at Stage 11. Claude CLI reviewer switches BACK to ADVERSARIAL mode for all stages from here on.
...
5. Claude CLI collaborative additions append revival candidates and identify alternates now
...
8. PLAN_CODE_AUDIT.md verdict line is `MATCHES_PLAN` or scoped `PARTIAL_MISMATCH` whose
   missing pieces are irrelevant to this scale-up wave (G11). `CRITICAL_MISMATCH` and
   `ERROR` block until the mismatch, incomplete input set, or unanswerable regime is fixed
   or explicitly judged by a human. Claude CLI reviewer unavailability is a LOUD STOP before audit
...
```text
Before proceeding to the next major stage, produce an agent recommendation. For high-risk
transitions, the gate is a human-authored or human-confirmed Human Decision Note; do not
silently write one as if the human approved.
...
Write AGENT_DECISION_RECOMMENDATION.md. Only write HUMAN_DECISION_NOTE.md when the user
has explicitly supplied or confirmed the decision, and include the human decision source in
the note.
```
...
| 4 | 11 | Claude CLI reviewer switches back to adversarial here |
