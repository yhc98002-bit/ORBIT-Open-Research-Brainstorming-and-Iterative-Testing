...
| **Innovation** | 8, 9, 10, 18.5 | Divergent mechanism invention, analogy / cross-pollination, algorithm sketch tournament, failure-to-innovation. **Claude CLI reviewer switches to collaborative mode here** — see `innovation-loops.md`. |
...
8     Mechanism Invention Loop                        (innovation: Claude CLI collaborative)
9     Analogy / Cross-pollination Loop                (innovation: Claude CLI collaborative)
10    Algorithm Sketch Tournament                     (innovation: Claude CLI collaborative)
...
18.5  Failure-to-Innovation Loop                      (innovation: Claude CLI collaborative)
...
Bold analogies welcome. Claude CLI collaborative.
...
Claude CLI reviewer switches back to **adversarial mode**. Every committed experiment must specify
...
Claude CLI reviews the implementation as a semantic auditor. Checks whether training/eval
...
Claude CLI collaborative. Anti-pattern: presenting the new mechanism as a pre-planned hypothesis
(violates G17).
...
`HUMAN_DECISION_NOTE.md` with a final verdict line of `PROCEED` (G15/G19). Agent-written
recommendations are useful context but do not satisfy this gate.
...
Before major transitions, the agent may write `AGENT_DECISION_RECOMMENDATION.md` describing
what is believed, what evidence supports it, what remains uncertain, and what it recommends.
For high-risk transitions, a human-authored or human-confirmed `HUMAN_DECISION_NOTE.md` is
the gate artifact; the agent must not silently fabricate human approval.

Required artifact: `AGENT_DECISION_RECOMMENDATION.md` for agent recommendation.
Required gate artifact: `HUMAN_DECISION_NOTE.md` ending `PROCEED` for scale-up, paper
writing, or public release.
...
G11 IF PLAN_CODE_AUDIT.md verdict = CRITICAL_MISMATCH
    ...
    No exception. ERROR means the audit did not complete because required inputs were
    incomplete or internally inconsistent; block any run that would rely on the failed
    audit until the cause is fixed or explicitly judged by a human. Claude CLI reviewer unavailability
    follows claude-cli-review.md: LOUD STOP, STATE awaiting_user_action, and no substitute
    audit verdict.
...
G15 IF scale-up requested AND (mode = COMMITMENT OR risk_score ≥ 4)
    THEN HUMAN_DECISION_NOTE.md with final verdict PROCEED is required before
...
For Stages 8, 9, 10, 18.5 (Innovation), Claude CLI reviewer switches to **collaborative mode** — see
...
For ORBIT, Claude CLI review defaults are:
