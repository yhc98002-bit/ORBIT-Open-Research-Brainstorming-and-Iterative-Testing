...
Claude CLI review remains required for this audit. If the Claude CLI reviewer path fails,
do not write a local substitute `PLAN_CODE_AUDIT.md`. Export a standalone prompt per
`../shared-references/claude-cli-review.md` §5.5:
...
Then stop with `pause_reason: claude_review_needed` and safe next command
