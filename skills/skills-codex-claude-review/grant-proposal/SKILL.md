...
**What this does:**
- Claude CLI max-effort acts as a grant review panelist (not a paper reviewer)
...
**What this does:**
- Claude CLI max-effort acts as a grant review panelist
...
> ⚠️ **Claude CLI reviewer fallback**: If `claude -p` is not available (no OpenAI API key), skip external review. Note "External review skipped — no Claude CLI reviewer available. Consider running `/auto-review-loop-llm` separately." in GRANT_REVIEW.md. The proposal is still usable without external review.

If `/research-review` is invoked (preferred), it handles the Claude CLI review call internally. If calling Codex directly (e.g., to maintain thread context from Phase 2):
...
| `reviewer model` | claude-cli | Claude CLI review model | → Claude CLI reviewer |
