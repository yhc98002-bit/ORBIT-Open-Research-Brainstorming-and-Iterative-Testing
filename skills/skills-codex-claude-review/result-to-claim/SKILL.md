...
`../shared-references/claude-cli-review.md`; do not accept a local
single-model substitute as satisfying this gate.

If Claude CLI reviewer is unavailable before or during this judgment, export a
...
python3 direct Claude CLI prompt/raw-JSON files generate \
  ...
  --role "Independent Claude CLI reviewer judging whether results support paper-bearing claims" \
  ...
  --output-artifact "orbit-research/CLAUDE_RESULT_TO_CLAIM_REVIEW.md" \
  --current-stop "STOP_C" \
...
This writes `.aris/review-prompts/result-to-claim.claim-evaluation.md`
...
Do not mark `claims/claim_ledger.json` as `ready` until a Claude CLI reviewer response exists or
After fixing Claude CLI access, rerun the blocked skill with its documented resume flag.
...
# Claude CLI reviewer per-call config does not accept a sandbox key — see ../shared-references/reviewer-routing.md.
  prompt: |
...
- `status: "draft"` when evidence is still being reconciled or Claude CLI review is pending.
...
- If a draft ledger is written while waiting for standalone Claude CLI import, it must include
...
- If the user explicitly passes `— claude-required: false`, every output must carry visible
...
- If Claude CLI reviewer is unavailable or a Claude CLI review call fails, use
After fixing Claude CLI access, rerun the blocked skill with its documented resume flag.
  `orbit-research/ORBIT_STATE.json` with producer context, `pause_reason:
  "claude_review_needed"`, and the import command. Keep the literal state marker
  `pause_reason: "claude_review_needed"` visible for tooling. After import,
