...
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, Agent, WebSearch, mcp__codex-image2__generate, mcp__codex-image2__generate_start, mcp__codex-image2__generate_status
---
...
- **OPTIONAL_TEXT_CRITIC = `claude -p`** — Optional text-only second opinion for layout/style checks
...
If `claude -p` is available, you may ask it for a short second-opinion
...
If `claude -p` is available, you may ask it for a short text-only
