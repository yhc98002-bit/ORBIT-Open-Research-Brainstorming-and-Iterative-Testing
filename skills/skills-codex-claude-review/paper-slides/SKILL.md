...
### Phase 5: Claude CLI reviewer Review

Send the slide outline + selected LaTeX frames to Claude CLI max-effort:
...
> ⚠️ If `claude -p` is not available (no OpenAI API key), skip external review and proceed to Phase 6. Note the skip in `SLIDES_STATE.json`.
