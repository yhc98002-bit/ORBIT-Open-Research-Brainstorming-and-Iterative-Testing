...
- `../shared-references/innovation-loops.md` — Stages 8/9/10/18.5 procedures + Claude CLI collaborative mode
...
- Claude CLI availability and standalone handoff: `../shared-references/claude-cli-review.md`
...
- **CLAUDE_REVIEW_MODEL = `claude-cli`** — Default Claude CLI reviewer for ORBIT.
- **CLAUDE_REVIEW_EFFORT = `xhigh`** — Mandatory for all ORBIT review gates.
- **CLAUDE_SANDBOX_MODE = `danger-full-access`** — Set globally in `~/.codex/config.toml`,
  not per call. Claude CLI reviewer `config` only accepts `Claude CLI `--effort max``; do not try to
  pass `sandbox` per call.
- **REVIEWER_INDEPENDENCE = on** — Pass file paths and objective, not executor summaries.
- **CLAUDE_INNOVATION_MODE** — `COLLABORATIVE` at Stages 8, 9, 10, 18.5 (use template in
...
orbit-research/MECHANISM_IDEATION.md          (innovation: Claude CLI collaborative)
orbit-research/ANALOGY_TRANSFER.md            (innovation: Claude CLI collaborative)
orbit-research/ALGORITHM_TOURNAMENT.md        (innovation: Claude CLI collaborative)
...
orbit-research/FAILURE_TO_INNOVATION.md       (when triggered, innovation: Claude CLI collaborative)
...
**Codex mode switch:** `CLAUDE_INNOVATION_MODE = COLLABORATIVE`. Use template in
`innovation-loops.md` §7.1. Claude CLI reviewer appends candidates; does NOT veto.
...
top-3 marked for Stage 9, and a "Claude CLI collaborative additions" section.
...
### Stage 11: Hypothesis-Mechanism-Benchmark-Control Matrix  *(Claude CLI reviewer switches BACK to ADVERSARIAL)*
...
- `ERROR` means the audit did not complete because required inputs were incomplete or a
  ...
  the cause is fixed or explicitly judged by a human. Claude CLI reviewer unavailability follows
  `claude-cli-review.md`: LOUD STOP, STATE `awaiting_user_action`, and no substitute
  audit verdict.
...
- `CLAUDE_INNOVATION_MODE = COLLABORATIVE` for these stages. Use prompt template in
...
- Claude CLI reviewer switches back to `ADVERSARIAL` at Stages 11, 14, 15, 17, 21, 23.
...
G11 PLAN_CODE_AUDIT.md verdict = CRITICAL_MISMATCH blocks scale-up unconditionally. Loop
    ...
    Claude CLI reviewer unavailability is a LOUD STOP via claude-cli-review.md, not an ERROR verdict.
