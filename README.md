...
- **2026-03-22** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔍 **[Codex + Gemini review guide](docs/CODEX_GEMINI_REVIEW_GUIDE.md)** — Codex executes, Gemini reviews via the local `gemini-review` MCP bridge plus a thin `skills-codex-gemini-review` overlay. Direct Gemini API is the default backend, so the original ARIS review-heavy skills can be reused with minimal changes. [CN](docs/CODEX_GEMINI_REVIEW_GUIDE_CN.md)
...
> **Want Codex to execute but Gemini to review locally?** See [`docs/CODEX_GEMINI_REVIEW_GUIDE.md`](docs/CODEX_GEMINI_REVIEW_GUIDE.md) and [CN](docs/CODEX_GEMINI_REVIEW_GUIDE_CN.md). That path installs the base `skills/skills-codex/*`, then overlays `skills/skills-codex-gemini-review/*`, and routes review-heavy skills through the local `gemini-review` MCP bridge using direct Gemini API by default.
